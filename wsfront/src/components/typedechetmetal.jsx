...
import { getAllMetalWaste, deleteMetalWaste } from "../services/typedechetService";
...
export default function MetalWasteListPage() {
...
const [modalOpen, setModalOpen] = useState(false);
  ...
  const fetchMetalWastes = async () => {
    try {
      setLoading(true);
      const data = await getAllMetalWaste();
      setWastes(data);
    } catch (error) {
      console.error("Erreur lors de la récupération des MetalWaste :", error);
    } finally {
      setLoading(false);
    }
  };
 const handleViewDechets = async (typeID, typeAppareil) => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce déchet métallique ?")) {
      try {
        await deleteMetalWaste(id);
        fetchMetalWastes();
      } catch (error) {
    ...
    <div className="page-content">
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-4xl font-bold text-gray-800">Liste des MetalWaste</h2>
          <button
            onClick={() => navigate("/add-metalwaste")}
            ...
            <table className="w-full">
              <thead className="bg-gray-800 text-white">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left font-semibold">Type de Métal</th>
                ...
                {wastes.length > 0 ? (
                  wastes.map((waste, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">{waste.typeMetal || "—"}</td>
                      ...
                      <button
                        ...
                        <button
                          onClick={() => navigate(`/edit-metalwaste/${waste.typeID}`)}
                  ...
                  <tr>
                    <td colSpan="5" className="text-center py-6 text-gray-500 italic">
                      Aucun MetalWaste enregistré pour le moment.
                    </td>
                  </tr>
                )}
              </tbody>
